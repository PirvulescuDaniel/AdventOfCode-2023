...
int main()
{
  ...
  {
    const int linesDistance = std::abs(p1.first - p2.first) + std::abs(emptyLines[p2.first] - emptyLines[p1.first]);
    const int columnDistance = std::abs(p1.second - p2.second) + std::abs(emptyColums[p2.second] - emptyColums[p1.second]);
